    ...
    <template #page-content-wrap>
      ...
      <div class="call-content">
        ...
        <div class="call-active" v-if="localCallStatus === LOCAL_CALL_STATUS.LOCAL_CALL_CALLING">
          <div class="agora-remote" ref="playerFull"></div>
          <!-- 本地视频容器（带背景） -->
          <div class="agora-local-wrapper">
            <div class="agora-local-bg"></div>
            <div class="agora-local" ref="playerMin" v-show="cameraEnabled && initialMediaStateApplied"></div>
          </div>
          ...
          <div class="local-control-panel" :class="{'local-control-panel-all-show': !cameraEnabled && !micEnabled}">
            ...
            <!-- 通话计时器 -->
            <div class="call-timer" v-if="callStartTime > 0">
              {{ callDurationText }}
            </div>
          </div>
          <!-- 顶部半透明渐变背景 -->
                ...
                <div class="msg-content">
                  ...
                  <div v-if="isCustomMessage(it)">
                    <div class="msg-gift" v-if="isGiftMessage(it)">
                      <!-- <img :src="it.customData.content?.staticPic || it.customData.image"/> -->
        ...
        <div v-if="localCallStatus === LOCAL_CALL_STATUS.LOCAL_CALL_END" class="call-end-overlay">
          <RechargeCallEnd v-if="hasFirstRecharge" @close="handleEndPageClose"/>
          <CallEnd v-else @close="handleEndPageClose"/>
        </div>
      </div>
      </div>
    </template>
...
import AgoraRTMManager, { RTM_MSG_TYPE } from "@/utils/AgoraRTMManager";
...
export default {
  ...
  data() {
    return {
      ...
      // 视频轨刷新定时器（用于解决主播端网络慢/VPN 下黑屏，用户端自动刷新推流）
      videoRefreshTimer: null,
      // 美颜开关状态（默认开启）
      beautyEnabled: true,
      // 美颜参数配置
      beautyOptions: {
        smoothnessLevel: 0.5,      // 平滑度（磨皮）
        lighteningLevel: 0.7,      // 亮度（美白）
        rednessLevel: 0.1,         // 红色度（红润）
        lighteningContrastLevel: 1 // 对比度
      },
      // 进入通话结束页时冻结「显示 RechargeCallEnd 还是 CallEnd」，避免 RechargeCallEnd 内 FetchFirstRechargeList 更新 allPurchased 导致闪一下又变成 CallEnd
      showRechargeCallEndWhenEnd: false,
    }
  },
  watch: {
    localCallStatus(newStatus, oldStatus) {
      if (newStatus === LOCAL_CALL_STATUS.LOCAL_CALL_END && oldStatus !== LOCAL_CALL_STATUS.LOCAL_CALL_END) {
        // 进入结束态时按当前 allPurchased 决定一次，之后不再随 store 变化而切换
        this.showRechargeCallEndWhenEnd = !this.$store.state.user.allPurchased;
        console.log('[🎥CALL_FLOW] 通话结束，冻结结束页类型', { showRechargeCallEndWhenEnd: this.showRechargeCallEndWhenEnd });
      } else if (newStatus !== LOCAL_CALL_STATUS.LOCAL_CALL_END) {
        this.showRechargeCallEndWhenEnd = false;
      }
    }
  },
  computed: {
    /** 有首充：未视为已充值（!allPurchased）时显示 RechargeCallEnd，否则 CallEnd */
    hasFirstRecharge() {
  ...
  created() {
    console.log('[🎥CALL_FLOW] ====== 进入 calling 页面 ======', new Date().toLocaleString('zh-CN', { hour12: false }));
    console.log('[🎥CALL_FLOW] 页面创建时间:', new Date().toLocaleTimeString('zh-CN', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit', fractionalSecondDigits: 3 }));
    // 初始化腾讯 IM 消息监听
    this.initData();
  ...
  methods: {
    ...
    /**
     * 判断消息是否是自己发送的（兼容 RTM 和腾讯 IM）
     */
    isMessageFromSelf(message) {
      if (!message) return false;
      // RTM 消息
      if (message.isRTMMessage) {
        return AgoraRTMManager.isSelf(message.from);
      }
      // 腾讯 IM 消息
    ...
    /**
     * 判断是否是文本消息（兼容 RTM 和腾讯 IM）
     */
    isTextMessage(message) {
      if (!message) return false;
      // RTM 消息
      if (message.isRTMMessage) {
        return message.type === RTM_MSG_TYPE.TEXT;
      }
      // 腾讯 IM 消息
    ...
    /**
     * 判断是否是自定义消息（兼容 RTM 和腾讯 IM）
     */
    isCustomMessage(message) {
      if (!message) return false;
      // RTM 消息
      if (message.isRTMMessage) {
        return message.type === RTM_MSG_TYPE.CUSTOM;
      }
      // 腾讯 IM 消息
    ...
    isGiftMessage(message) {
      ...
      // RTM 礼物消息类型是 RTM_MSG_TYPE.GIFT (2000)
      // 腾讯 IM 礼物消息类型是 custom_type_gift (2000)
      return customType === RTM_MSG_TYPE.GIFT || customType === custom_type_gift;
    },
    initData() {
    ...
    initParams() {
      console.log('[🎥CALL_FLOW] initParams 初始化参数', new Date().toLocaleTimeString('zh-CN', { hour12: false }));
      if (this.localCallStatus !== LOCAL_CALL_STATUS.LOCAL_CALL_CALLING) {
        console.log('[🎥CALL_FLOW] ❌ 通话状态异常，返回上一页');
      ...
      console.log('[🎥CALL_FLOW] 开始初始化通话...', new Date().toLocaleTimeString('zh-CN', { hour12: false }));
      this.onCallStart();
      // 5秒后启用挂断按钮
    ...
    initTencentImMessageListener() {
      // 腾讯 IM 监听：只用于通话状态相关消息（保留通话业务逻辑）
      TencentImUtils.onMessageReceived(this.onTencentImMessageReceived);
      TencentImUtils.onMessageModified(this.onMessageModified);
    },

    /**
     * 初始化 RTM 消息监听（用于聊天消息收发）
     * 注意：RTM 登录已在应用启动时完成，这里只需要加入频道
     */
    async initRTMMessageListener() {
      try {
        const channelName = this.getCallNo();

        console.log('[🎥CALL_FLOW] 加入 RTM 频道...', { channelName });

        // 检查 RTM 是否已登录
        if (!AgoraRTMManager.checkLoggedIn()) {
          console.warn('[🎥CALL_FLOW] RTM 未登录，尝试登录...');
          const { DEF_AGORA_APP_ID } = store.state.PageCache.configData;
          const userId = this.loginUserInfo.userId;
          await AgoraRTMManager.login(DEF_AGORA_APP_ID, userId);
        }

        // 加入 RTM 频道
        await AgoraRTMManager.joinChannel(this.loginUserInfo.userId.toString());

        // 设置 RTM 消息接收回调
        AgoraRTMManager.onMessageReceived(this.onRTMMessageReceived);

        console.log('[🎥CALL_FLOW] ✅ RTM 频道加入成功');
      } catch (error) {
        console.error('[🎥CALL_FLOW] RTM 频道加入失败:', error);
        // RTM 初始化失败不影响通话，只是聊天功能不可用
      }
    },
     ...
     * RTM 消息接收处理（聊天消息）
     */
    onRTMMessageReceived(event) {
      const messageList = event.data || [];
      messageList.forEach((message) => {
        console.log('[🎥CALL_FLOW] 收到 RTM 消息:', message);

        // 处理文本消息
        if (message.type === RTM_MSG_TYPE.TEXT) {
          // 不处理自己发送的消息（发送时已本地添加）
          if (!AgoraRTMManager.isSelf(message.from)) {
        ...
        // 处理自定义消息（如礼物）
        if (message.type === RTM_MSG_TYPE.CUSTOM) {
          const customType = message.customData?.customType || message.customData?.type;
          if (customType === RTM_MSG_TYPE.GIFT) {
            this.messageList.push(message);
            this.scrollToBottom();
          }
        }
    ...
    onTencentImMessageReceived(event) {
      const messageList = event.data;
      messageList.forEach((message) => {
        console.log("[🎥CALL_FLOW] 收到腾讯 IM 消息:", message);
    ...
    onTopMoreClick() {
      ...
      showUserDetailMoreDialog({
        ...
        onBlockAndReport: async () => {
          // 先拉黑用户，然后显示举报对话框
          // const rsp = await statusBlack(this.userId, true);
          // if (rsp && (rsp.success || rsp.code === 200 || rsp.code === 0)) {
          //   showCallToast('User blocked successfully');
          //   // 显示举报对话框
            const userInfo = this.anchorInfo && Object.keys(this.anchorInfo).length > 0 ? {
    ...
    async toggleCamera() {
      ...
      await this.localTracks.videoTrack.setEnabled(newState);
    ...
    async toggleMic() {
      ...
      await this.localTracks.audioTrack.setEnabled(newState);
    ...
    async switchCamera() {
      ...
      await this.localTracks.videoTrack.setDevice(nextDevice.deviceId);
      // 更新 store 中的摄像头方向状态
    ...
    setVideoAttributes(container) {
      ...
      // 等待 video 元素被创建（Agora SDK 可能异步创建）
    ...
    initCallStartTimer() {
      ...
      if (this.pendingRechargeReminder) {
        console.log('[🎥CALL_FLOW] 处理待处理的充值提醒');
    ...
    async ensureMediaPermissions() {
      ...
      try {
        ...
        // showHintDialog('Please allow camera/microphone permission.', () => true);
        showConfirmDialog({
    ...
    async initDevices() {
      console.log('[🎥CALL_FLOW] initDevices 开始初始化设备', new Date().toLocaleTimeString('zh-CN', { hour12: false }));

      const permissionOk = await this.ensureMediaPermissions();
      if (!permissionOk) {
        console.log('[🎥CALL_FLOW] ❌ 媒体权限获取失败');
        return;
      }
      console.log('[🎥CALL_FLOW] ✅ 媒体权限获取成功');


      // 使用 store 中的用户数据
      const {DEF_AGORA_APP_ID} = store.state.PageCache.configData;
      let rtcToken = this.callInfo.createUserRtcToken;
      const appId = DEF_AGORA_APP_ID;
      const token = rtcToken;
      const channel = this.getCallNo();
      const uid = this.loginUserInfo.userId;

      console.log('RTC 配置', { appId, channel, uid });

      // 使用 AgoraRTCManager 单例加入房间并发布流
      const { rtcClient, localTracks } = await AgoraRTCManager.joinAndPublish(
        appId,
        channel,
        token,
        uid,
        {
          onUserPublished: async (user, mediaType) => {
            console.log('[🎥CALL_FLOW] ====== 收到远程用户发布流 ======', new Date().toLocaleTimeString('zh-CN', { hour12: false }));
            console.log('[🎥CALL_FLOW] 远程用户信息:', { userId: user.uid, mediaType });
            // 使用 AgoraRTCManager 获取 rtcClient（因为 this.rtcClient 在回调时可能还未赋值）
            const client = AgoraRTCManager.getRtcClient();
            await client.subscribe(user, mediaType);
            console.log('[🎥CALL_FLOW] ✅ 订阅远程用户流成功', { userId: user.uid, mediaType });
            if (mediaType === "video") {
              console.log('[🎥CALL_FLOW] 开始播放远程视频流', user.uid);
              user.videoTrack.play(this.$refs.playerFull);
              this.$nextTick(() => {
                this.setVideoAttributes(this.$refs.playerFull);
                this.remoteVideoReady = true;
                console.log('[🎥CALL_FLOW] ✅ 远端视频流就绪，开始显示', new Date().toLocaleTimeString('zh-CN', { hour12: false }));
              });
            }
            if (mediaType === "audio" && user.audioTrack) {
              user.audioTrack.play();
              console.log('[🎥CALL_FLOW] ✅ 远端音频流开始播放', new Date().toLocaleTimeString('zh-CN', { hour12: false }));
            }
            ...
            // }, 300);
            // 主播端网络慢/VPN 时可能黑屏：用户端自动“模拟切换摄像头”触发重新推流
            setTimeout(() => {
              this.refreshVideoTrackForHost();
            }, 300);
          },
          onUserUnpublished: () => {
            console.log('end ===================== user-unpublished');
          },
          onException: (evt) => {
      ...
      this.beautyEnabled = AgoraRTCManager.beautyEnabled;

      // 播放本地视频预览
      console.log('[🎥CALL_FLOW] 开始播放本地视频预览');
      this.localTracks.videoTrack.play(this.$refs.playerMin);
      this.$nextTick(() => {
        this.setVideoAttributes(this.$refs.playerMin);
        console.log('[🎥CALL_FLOW] ✅ 本地视频预览就绪');
      });
      ...
      // 初始化 RTM 用于聊天消息收发
      this.initRTMMessageListener();

      console.log('[🎥CALL_FLOW] ====== 本地初始化完成，等待远程用户加入 ======', new Date().toLocaleTimeString('zh-CN', { hour12: false }));
    },
    // 进入房间后应用初始的摄像头和麦克风状态
    async applyInitialMediaState(isP = false) {
      ...
      if (this.localTracks.videoTrack) {
        ...
        this.localTracks.videoTrack.setEnabled(cameraState).then(() => {
      ...
      if (this.localTracks.audioTrack && isP === false) {
        const micState = this.micEnabled;
        this.localTracks.audioTrack.setEnabled(micState).then(() => {
    ...
    /**
     * 刷新本地视频轨（先关再开），解决主播端网络慢/VPN 进入时黑屏问题。
     * 主播端无法改代码时，用户端通过自动“模拟切换摄像头开关”触发重新推流，主播端即可收到流。
     */
    async refreshVideoTrackForHost() {
      if (!this.localTracks?.videoTrack) return;
      ...
      try {
        ...
        console.log('[🎥CALL_FLOW] 已自动刷新视频轨，便于主播端收流');
      } catch (err) {
        console.warn('[🎥CALL_FLOW] 刷新视频轨失败:', err);
      }
    },
    ...
    async onCallEnd(status, isMyOperate = false, callTime = 0, is) {
      ...
      // await this.$store.dispatch('call/setlocalCallStatus', LOCAL_CALL_STATUS.LOCAL_CALL_END);
      // // 更新通话状态为结束
      // await this.$store.dispatch('call/setCallStatus', CALL_STATUS.CALL_DONE.value);
      try {
        const res = await callEnd(this.getCallNo(), status);
        // 从 API 返回的数据中获取首充信息（字段名待确认，根据实际 API 返回调整）
        if (res && res.data) {
          // TODO: 根据实际 API 返回的字段名调整，可能是 firstFlag、needFirstRecharge、hasFirstRecharge 等
          // 示例：this.hasFirstRecharge = res.data.firstFlag === false;
          // 或者：this.hasFirstRecharge = res.data.needFirstRecharge === true;
          // 暂时先注释，等字段确定后再启用
          // this.hasFirstRecharge = res.data.firstFlag === false;
          console.log('[🎥CALL_FLOW] 通话结束 API 返回数据:', res.data);
        }
      } catch (error) {
        console.error('[🎥CALL_FLOW] 通话结束 API 调用失败:', error);
      }
    },
    ...
    /**
     * 从 API 获取首充状态
     * TODO: 根据实际 API 接口和返回字段调整
     */
    async fetchFirstRechargeStatus() {
      try {
        // TODO: 调用实际的 API 获取首充信息
        // 示例：const res = await apiGetFirstRechargeStatus();
        // 或者从 callEnd API 的返回中获取（已在 onCallEnd 中处理）
        // 或者从通话结束消息的 customData 中获取（已在 onTencentImMessageReceived 中处理）
        ...
        console.log('[🎥CALL_FLOW] 获取首充状态（待实现）');
      } catch (error) {
        console.error('[🎥CALL_FLOW] 获取首充状态失败:', error);
      }
    },
    async handleEndPageClose() {
      // 先销毁 TRTC 等资源，再离开页面；即使销毁失败也执行返回
      try {
        await this.destroy();
      } catch (error) {
    ...
    translateMessage(message, index) {
      ...
      translationMsg(message).then((result) => {
        ...
        // 翻译成功后，消息会通过 onMessageModified 事件更新
      }).catch((error) => {
    ...
    async sendTextMessage() {
      ...
      // tencentChat.m_sendTextMessage(this.anchorInfo.userId, text, (data) => {
      //   console.log("=========",data);
      //   if (data.funnyMessage) {
      //     this.sendRtmJson(JSON.stringify(data.funnyMessage));
      //   }
      // }, (error) => {
      //
      // });
      await this.sendRtmJson(text)
    },

    async sendRtmJson(text) {
      // 立即清空输入框，防止重复发送
      this.textContent = '';
      console.log('[🎥CALL_FLOW] sendTextMessage via RTM:', text);

      try {
        // 使用 RTM 发送消息
        const sentMessage = await AgoraRTMManager.sendTextMessage(text);
        // 将发送的消息添加到本地列表
        this.messageList.push(sentMessage);
        this.scrollToBottom();
        this.isSendingMessage = false;
      } catch (error) {
        console.error('[🎥CALL_FLOW] RTM 发送消息失败:', error);
        this.isSendingMessage = false;
    ...
    async destroy() {
      ...
      // 销毁 RTM 资源（聊天消息相关）
      try {
        await AgoraRTMManager.destroy();
      } catch (e) {
        console.error('[🎥CALL_FLOW] RTM 销毁出错:', e);
      }

      // 使用 AgoraRTCManager 销毁资源
      await AgoraRTCManager.destroy();
...
.call-content {
  ...
  .local-control-panel {
    ...
    z-index: 6;
    right: 0px;
    bottom: 100px; /* 与 agora-local 相同位置 */
    width: 100px;
    height: 150px; /* 与 agora-local 相同高度 */
    border-radius: 14px;
    padding: 12px;
    display: flex;
    flex-direction: column;
  ...
  .chat {
    position: absolute;
    left: 0;
    right: 130px; /* 避开右边的 local 视频 */
    bottom: 110px; /* 给 chat-input 留出空间 */
    box-sizing: border-box;
    pointer-events: auto;
  ...
  .item {
    margin-bottom: 10px;
    width: 100%;
    min-width: 0; /* flex 子项可收缩，避免撑开父级 */
    box-sizing: border-box;
  }

  /* 所有消息统一气泡样式，仅文字颜色区分：自己发送为黄色，收到为白色 */
  .msg-content {
    display: inline-block;
    width: auto;
    max-width: 78%;
  ...
  /* 自己发送的消息：文字为黄色；收到消息无此类，文字为白色（.msg-text 默认色） */
  .msg-text-self {
    color: #FFDB0D;
  }
